...
import { Home as HomeIcon, Search as SearchIcon, Settings as SettingsIcon, LayoutDashboard, LogIn, Bell, Sun, Moon, MessageCircle, User as UserIcon, MapPin, Navigation, Code2, X, Mail, User as DevIcon } from 'lucide-react';
import { Currency, AppSettings, Store, User, UserRole, UserStatus, Category, Province, StorePost, AppTheme, ActionLog, Comment, Report, InternalMessage, Coordinates } from './types';
...
const App: React.FC = () => {
  ...
  const [users, setUsers] = useState<User[]>(() => {
    ...
    const adminExists = existingUsers.find((u: User) => u.username === 'ادمن');
  ...
  useEffect(() => {
    ...
    // تحديث متغيرات CSS في المتصفح
    document.documentElement.style.setProperty('--theme-primary', themeColors.hex);
    document.documentElement.style.setProperty('--theme-ripple', themeColors.hex + '4D'); // 30% alpha
  ...
  return (
    <Router>
      <div className={`flex flex-col min-h-screen pb-20 md:pb-0 md:pt-16 transition-all duration-500 ${settings.isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-gray-50 text-gray-900'}`}>
        ...
        {/* زر المطور للجوال (يضاف إلى الهيدر العلوي في الجوال أو يظهر بجانب البحث) */}
        ...
        <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-6">
          <Routes>
            ...
            />} />
            <Route path="/category/:id" element={<CategoryPage stores={stores} categories={categories} themeColors={themeColors} />} />
